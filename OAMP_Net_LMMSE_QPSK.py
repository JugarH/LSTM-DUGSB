...
#parameters
reuse=tf.AUTO_REUSE
# reuse = 'AUTO_REUSE'  # 或者 tf.AUTO_REUSE
N = 32
M = 32
snrdb_train = 25 # training SNR
...
# snr_test=10
weight_mat='/home/huangzujia/jugarh/SB/mimo/OAMP'
...
# def Test_batch(sess, snr_test):
#     _loss = []
#     _ber = []
#     bit_err_total = 0

#     while bit_err_total < errsum:
#         batch_Y, batch_H, batch_X = generate_data_iid_test(
#             batch_size, M, N, snr_test
#         )

#         b_loss, b_ber, bit_err_num, s_ = sess.run(
#             [cost, ber, bit_err_sum, s],
#             {Y_: batch_Y.T, A_: batch_H, X_: batch_X.T}
#         )

#         _loss.append(b_loss)
#         _ber.append(b_ber)
#         bit_err_total += bit_err_num

#     return np.mean(_loss), np.mean(_ber), s_
...
#     while bit_err_total < errsum:
#         batch_Y, batch_H, batch_X = generate_data_iid_test(
#             batch_size, M, N, ebn0_test
#         )

#         b_loss, bit_err_num = sess.run(
#             [cost, bit_err_sum],   # 注意：用 bit_err_sum
#             {Y_: batch_Y.T, A_: batch_H, X_: batch_X.T}
#         )
...
#         bit_err_total += bit_err_num
#         bit_total += batch_size * 2 * N   # QPSK: 每实维 1 bit

#     ber = bit_err_total / bit_total

#     return np.mean(_loss), ber
...
def Test_batch(sess, ebn0_test):
    ...
    # ===== 新增：BER 标准差 =====
    std_ber = np.sqrt(ber * (1 - ber) / bit_total)

    return np.mean(_loss), ber, std_ber
...
def Test(snr_test):
    with tf.Session() as sess:
        ...
        loss_test, ber_test, std = Test_batch(sess, snr_test)

        print("SNR: %.1f dB, Test Loss: %.6f, BER: %.6e ± %.6e"
              % (snrdb_test, loss_test, ber_test, std))

    return loss_test, ber_test, std
...
#        tau2=M/(M-N)*sigma2*tf.ones((batch_size,))
    ...
    # ===== 新增：BER（bit-level）=====
    s_bit = tf.cast(s > 0, tf.float32)
    x_bit = tf.cast(X_ > 0, tf.float32)

    bit_err = tf.to_float(tf.not_equal(s_bit, x_bit))
    ber = tf.reduce_mean(bit_err)
    bit_err_sum = tf.reduce_sum(bit_err)
    ...
    loss_test, ber_test, std =Test(snr_train)
